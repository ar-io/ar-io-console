...
export class ServiceWorkerMessenger {
  ...
  private async doRegister(scriptURL: string, options?: RegistrationOptions): Promise<void> {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        console.log('[SW] No controller after registration, requesting claim...');
        ...
        if (swRegistration.active) {
          try {
            await new Promise<void>((resolve, reject) => {
              ...
              swRegistration.active!.postMessage({ type: 'CLAIM_CLIENTS' }, [messageChannel.port2]);
            });
        ...
        throw new Error('Service worker registered but not controlling. Please refresh the page.');
      } catch (error) {
  ...
  async waitForReady(): Promise<boolean> {
    ...
    console.warn('[SW] No registration in progress, starting fallback registration');
    try {
