...
function detectFromExtension(url: string): {
  ...
  // Default: For unknown extensions, check if it looks like a path (has no extension)
  // Paths like "/" or "/app" are likely HTML apps, while files should be downloaded
  // If URL path ends with "/" or has no extension, treat as HTML
  try {
    const urlObj = new URL(url);
    const pathname = urlObj.pathname;
    const hasExtension = /\.\w+$/.test(pathname);
    if (!hasExtension || pathname.endsWith("/")) {
      // Likely an HTML app or route
      return {
  ...
  // Unknown file with extension - offer download
  return {
