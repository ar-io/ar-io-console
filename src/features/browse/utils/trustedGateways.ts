 ...
 */
export async function getTrustedGateways(
    count: number = 3,
): Promise<GatewayWithStake[]> {
    ...
    try {
        ...
        const gatewaysWithTotalStake = result.items
            .filter(
                (gateway: any) =>
                    gateway.status === "joined" && gateway.settings?.fqdn,
            )
            .map((gateway: any) => ({
        ...
        gatewaysWithTotalStake.sort(
            (a: GatewayWithStake, b: GatewayWithStake) =>
 ...
 * This fetches from turbo-gateway.com/ar-io/peers which returns active gateways.
 */
