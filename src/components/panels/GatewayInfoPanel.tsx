...
export default function GatewayInfoPanel() {
  ...
  const [devResourcesExpanded, setDevResourcesExpanded] = useState(false);
  ...
  // Build config-aware API doc links
  const uploadApiDocsUrl = `${currentConfig.uploadServiceUrl.replace(/\/$/, '')}/api-docs`;
  const paymentApiDocsUrl = `${currentConfig.paymentServiceUrl.replace(/\/$/, '')}/api-docs`;
    ...
    <div className="px-4 sm:px-6">
      ...
      <div className="flex items-start gap-3 mb-4 sm:mb-6">
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 py-2 rounded-full border border-border/20 text-foreground/80 hover:text-foreground hover:border-primary/50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                      ...
                      <PopoverPanel className="absolute right-0 z-10 mt-2 w-72 rounded-2xl bg-card border border-border/20 shadow-lg p-3">
                        <div className="text-xs">
                          ...
                          <p className="text-foreground/80 leading-relaxed">
                            ...
                            all payment service features: credits, fiat top-ups, and balance checking. Only
      ...
      {/* ─── Service Status ─── */}
      {(uploadServiceInfo || pricingInfo || gatewayInfo?.x402 || arIOGatewayInfo || arweaveNodeInfo) && (
        <div className="bg-card rounded-2xl border border-border/20 p-4 sm:p-6 mb-4 sm:mb-6">
          ...
          {uploadServiceInfo && (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 mb-6">
              ...
              <div className="bg-background rounded-2xl p-4">
                <div className="text-xs text-foreground/80 uppercase tracking-wider mb-1">Arweave Peers</div>
          ...
          {pricingInfo && (
            <div className="mb-6">
              ...
              {/* The one place the console says what Turbo is. Every charge
                  ...
                  sponsored ArNS actions) without ever being introduced. By
                  function, not category: "gateway" already means the read side
                  here — Browse and the gateway status above both use real
                  ar.io gateways — and Turbo is the write side. */}
              <p className="text-sm text-foreground/80 mb-3 max-w-prose">
                Set by Turbo, the upload and payment service behind this console:
                it bundles your files onto Arweave and issues the credits you
                spend here. Built by the team behind ar.io and ArDrive.
              </p>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                <div className="bg-background rounded-2xl p-4">
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-lg font-bold text-foreground capitalize">{arIOGatewayInfo.status}</span>
                  </div>
      ...
      <div className="bg-card rounded-2xl border border-border/20 mb-4 sm:mb-6">
        <button
          onClick={() => setDevResourcesExpanded(!devResourcesExpanded)}
          className="w-full flex items-center justify-between p-4 sm:p-6 text-left"
        >
          <div className="flex items-center gap-3">
          ...
          <ChevronDown
            className={`w-5 h-5 text-foreground/80 transition-transform ${devResourcesExpanded ? 'rotate-180' : ''}`}
          />
        </button>

        {devResourcesExpanded && (
          <div className="px-4 sm:px-6 pb-4 sm:pb-6 border-t border-border/20 pt-4">
            {/* Service Wallets */}
            {uploadServiceInfo && (
              <div className="mb-6">
                ...
                <div className="grid md:grid-cols-2 gap-3">
                  {Object.entries(uploadServiceInfo.addresses).map(([chain, address]) => (
                    <div
                      key={chain}
                      className="bg-background rounded-2xl p-3 flex justify-between items-center"
                    >
                      <div>
            ...
            <div>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="bg-background rounded-2xl p-4 hover:bg-background/80 transition-colors flex items-start gap-3"
                >
                ...
                <a
                  href="https://turbo-gateway.com/api-docs/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="bg-background rounded-2xl p-4 hover:bg-background/80 transition-colors flex items-start gap-3"
                >
