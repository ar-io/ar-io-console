...
export default function TryItNowPanel() {
  ...
  const { uploadStatuses, getStatusIcon, checkUploadStatus, statusChecking, formatFileSize } =
    useUploadStatus();
  ...
  const { login } = useLogin({
    onComplete: async ({ user }) => {
      ...
      const existingWallet = user?.linkedAccounts?.find((account) => account.type === 'wallet');
  ...
  useEffect(() => {
    if (waitingForWallet && privyWallets && privyWallets.length > 0) {
      const privyWallet = privyWallets.find(
        (w) => w.walletClientType === 'privy' || w.walletClientType === 'embedded' || !w.imported,
      );
      ...
      <div className="flex items-start gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-foreground/80">
            Upload a file for free. It will be stored permanently and accessible to anyone with the
            link.
          </p>
        </div>
      </div>
          ...
          {!selectedFile ? (
            ...
            >
              <div className="mb-4">
                ...
                <p className="text-sm text-foreground/80">
                  Max file size:{' '}
                  <span className="text-foreground font-medium">{formatFreeLimit(freeLimit)}</span>
                </p>
              </div>
              <input
                type="file"
            ...
            <div className="space-y-4">
              ...
              <div className="bg-card rounded-2xl p-4">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-foreground font-medium truncate mb-1">{selectedFile.name}</p>
              ...
              <p className="text-xs text-center text-foreground/80">
                Once uploaded, this file will be publicly accessible and cannot be removed. By
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-8">
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-foreground/80">
              Learn about the permanent cloud - decentralized storage and hosting that lasts forever
            </p>
          </div>
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-foreground/80">
              Add permanent uploads to your app, or deploy and host it on the permaweb
            </p>
          </div>
          <ArrowRight className="w-4 h-4 text-foreground/80 group-hover:text-foreground group-hover:translate-x-1 transition-all" />
        </a>
