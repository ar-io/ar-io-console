    ...
    const updateArNSRecord = useCallback(
        async (
            ...
            try {
                ...
                if (address) {
                    console.log(
                        "Refreshing ArNS state for updated name:",
                        name,
                    );
                    setTimeout(async () => {
                        try {
                            // Get fresh ANT state for just this name
                            const nameRecord = names.find(
                                (n) => n.name === name,
                            );
                            if (nameRecord) {
                                ...
                                const updatedTarget =
                                    freshState.Records?.["@"]?.transactionId;
                                ...
                                const updatedCacheNames = cachedNames.map(
                                    (cachedName) =>
                            ...
                            console.warn(
                                "Failed to refresh ANT state after update:",
                                error,
                            );
        ...
        async (name: string): Promise<ArNSName | null> => {
            ...
            try {
                ...
                if (address) {
                    ...
                    const existingIndex = cachedNames.findIndex(
                        (c) => c.name === name,
                    );
                    if (existingIndex >= 0) {
                ...
                console.error(
                    "Failed to fetch ANT details for",
                    name,
                    ":",
                    error,
                );
        ...
        async (name: string): Promise<boolean> => {
            ...
            try {
                ...
                updatedUndernames.forEach((undername) => {
                    const ttlSeconds =
                        freshState.Records?.[undername]?.ttlSeconds;
                    if (ttlSeconds !== undefined) {
                ...
                setOwnedArNSNames(address, updatedCacheNames);
                console.log(
                    "Successfully refreshed ArNS name:",
                    name,
                    freshState,
                );
                return true;
