                    ...
                    if (currentChainId !== expectedChainId) {
                        ...
                        try {
                            ...
                            if (switchError?.code === 4902) {
                                ...
                                const networkParams =
                                    getNetworkParams(expectedChainId);
                                if (networkParams) {
                                    await window.ethereum.request({
    ...
    const fetchArioBalance = useCallback(
        async (
            ...
            try {
                ...
                throw new Error(
                    "Unable to fetch ARIO balance. Please try again.",
                );
            }
        },
        [getCurrentConfig],
    ...
    const fetchArBalance = useCallback(
        async (
            ...
            try {
                ...
                const gatewayUrl =
                    config.tokenMap["arweave"] || config.arioGatewayUrl;
                if (gatewayUrl) {
                    try {
                        const url = new URL(gatewayUrl);
                        arweaveConfig.host = url.hostname;
                        arweaveConfig.port = url.port
                            ? parseInt(url.port)
                            : 443;
                ...
                throw new Error(
                    "Unable to fetch AR balance. Please try again.",
                );
            }
        },
        [getCurrentConfig],
    ...
    const fetchSolBalance = useCallback(
        async (
            ...
            try {
                ...
                throw new Error(
                    "Unable to fetch SOL balance. Please try again.",
                );
            }
        },
        ...
        async (
            ...
            try {
                ...
                const balanceInUsdc = Number(
                    ethers.formatUnits(balanceInSmallest, 6),
                ); // USDC has 6 decimals
    ...
    const fetchBalance = useCallback(async () => {
        ...
        try {
            ...
            switch (tokenType) {
                ...
                default:
                    throw new Error(
                        `Balance fetching not implemented for ${tokenType}`,
                    );
            }
