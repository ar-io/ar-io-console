...
const mergeTags = (
  ...
  const nonOverriddenDefaults = defaultTags.filter((dt) => !customTagNames.has(dt.name));
              ...
              events: {
                onProgress: (progressData: { totalBytes: number; processedBytes: number }) => {
                  const percentage = Math.round(
                    (progressData.processedBytes / progressData.totalBytes) * 100,
                  );
                  setUploadProgress((prev) => ({
                    ...prev,
    ...
    async (
      ...
      if (options?.cryptoPayment && selectedToken && options?.tokenAmount) {
        try {
          ...
          const txIdMatch =
            errorMessage.match(/submitFundTransaction\([^)]*\)['":]?\s*(0x[a-fA-F0-9]{64})/i) ||
      ...
      for (const file of files) {
        ...
        try {
          ...
          const uploadOptions =
            options?.cryptoPayment && selectedToken ? { customTags: options?.customTags } : options;
          ...
          const errorToShow =
            rawError.length < 200
              ? rawError
              : formatUploadError(error instanceof Error ? error : 'Unknown error');
