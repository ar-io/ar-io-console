...
export function useArNSPricing(): UseArNSPricingReturn {
    ...
    const getAffordableOptions = (
        credits: number,
    ): ArNSAffordabilityOption[] => {
        ...
        pricingTiers.forEach((tier) => {
            ...
            for (let years = 1; years <= 5; years++) {
                const yearKey =
                    `year${years}` as keyof typeof tier.pricesInCredits;
        ...
        const loadPricing = async () => {
            ...
            try {
                ...
                if (!arioUSDPrice || arioUSDPrice <= 0) {
                    throw new Error(
                        "Failed to fetch ARIO price from CoinGecko",
                    );
                }
