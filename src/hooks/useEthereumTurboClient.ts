...
export function useEthereumTurboClient() {
    ...
    const creatingSignerRef = useRef<Promise<CachedEthereumSigner> | null>(
        null,
    );
        ...
        async (
            ...
            const createSigner = async (): Promise<CachedEthereumSigner> => {
                ...
                if (privyWallet) {
                    ...
                    try {
                        const connectorClient = await getConnectorClient(
                            wagmiConfig,
                            {
            ...
            const turboConfig = {
                ...
                ...(config.tokenMap[tokenType as keyof typeof config.tokenMap]
                    ? {
                          gatewayUrl:
                              config.tokenMap[
                                  tokenType as keyof typeof config.tokenMap
                              ],
                      }
                    : {}),
            };
