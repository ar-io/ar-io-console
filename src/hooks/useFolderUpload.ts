...
export function useFolderUpload() {
  ...
  const [deployStage, setDeployStage] = useState<
    'idle' | 'uploading' | 'manifest' | 'updating-arns' | 'complete' | 'cancelled'
  >('idle');
  ...
  const [hashingStage, setHashingStage] = useState<'idle' | 'hashing' | 'complete' | 'cancelled'>(
    'idle',
  );
            ...
            events: {
              ...
              onProgress: ({
                ...
                setActiveUploads((prev) =>
                  prev.map((u) => (u.name === file.name ? { ...u, progress: percentage } : u)),
                );
              },
              onError: (error: any) => {
  ...
  const updateDeployStage = useCallback(
    (stage: 'idle' | 'uploading' | 'manifest' | 'updating-arns' | 'complete' | 'cancelled') => {
